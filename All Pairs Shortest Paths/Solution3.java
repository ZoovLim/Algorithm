...
class Solution3 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int test_case = 1; test_case <= 10; test_case++) {
            ...
            // k = 0 일 때, 즉 정점 i -> j 로 직접 연결되는 경로의 거리 초기화 (직접 연결되는 경로가 존재하지 않을 경우 고려) : theta(N^2)
            for (int i = 0; i <= N; i++) {
                for (int j = 0; j <= N; j++) {
                    if (i == j) d[i][j] = 0;
                    else d[i][j] = 10000;
            ...
            // k = 0 일 때, 각 간선의 가중치 초기화 : theta(E)
            for (int i = 0; i < E; i++) {
                d[U[i]][V[i]] = W[i];
            }

            // DP를 이용한 Floyd-Warshall 알고리즘 : theta(N^3)
            for (int k = 1; k <= N; k++) {
                for (int i = 1; i <= N; i++) {
                    for (int j = 1; j <= N; j++) {
